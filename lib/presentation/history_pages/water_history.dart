...
class WaterHistoryPage extends StatelessWidget {
  static const String routeNamed = "WaterHistoryPage";

  @override
  Widget build(BuildContext context) {
    return CustomScaffold(
      appBarTitle: 'Water History',
      body: StoreObserver<OrderStore>(
        builder: (OrderStore orderStore, BuildContext context) {
          ...
          if (orderStore.water.isEmpty)
            return Center(
              child: getTitleTex("No Order Yet"),
            );
          return ListView.builder(
              itemCount: orderStore.water.length,
              itemBuilder: (BuildContext context, index) {
                List<VendorRequest> hhList = orderStore.water.values.toList();
