...
part 'essentials_store.g.dart';

class EssentialsStore = _EssentialsStore with _$EssentialsStore;

abstract class _EssentialsStore with Store {
  @observable
    ...
    if (productMap.isEmpty) {
      ...
      try {
        Map<String, List<Product>> response = await getProductService
            .getProductList(collectionName: 'essentials');
        print("reponse in essentials store");
        print(response);
        productMap.addAll(response);
        isLoading = false;
      } catch (e) {
        isLoading = false;
        print("error in essentials store");
        print(e);
        throw e;
      }
    }
