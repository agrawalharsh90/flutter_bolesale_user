...
part 'fashion_store.g.dart';

class FashionStore = _FashionStore with _$FashionStore;

abstract class _FashionStore with Store {
  @observable
    ...
    if (productMap.isEmpty) {
      ...
      try {
        Map<String, List<Product>> response = await getProductService
            .getProductList(collectionName: 'fashion');
        print("reponse in fashion store");
        print(response);
        productMap.addAll(response);
        isLoading = false;
      } catch (e) {
        isLoading = false;
        print("error in fashion store");
        print(e);
        throw e;
      }
    }
