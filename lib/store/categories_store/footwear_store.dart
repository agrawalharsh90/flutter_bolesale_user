...
part 'footwear_store.g.dart';

class FootwearStore = _FootwearStore with _$FootwearStore;

abstract class _FootwearStore with Store {
  @observable
    ...
    if (productMap.isEmpty) {
      ...
      try {
        Map<String, List<Product>> response = await getProductService
            .getProductList(collectionName: 'footwear');
        print("reponse in footwear store");
        print(response);
        productMap.addAll(response);
        isLoading = false;
      } catch (e) {
        isLoading = false;
        print("error in footwear store");
        print(e);
        throw e;
      }
    }
